...
'''
    # 본 파일은 ObjectMemory, SceneMap, DynamicGlobalSceneGraphManager 클래스로 구성됨.
    # ObjectMemory는 정적 GSG 생성 연구에서 사용함. 현재까지 발견한 Object 정보들을 저장하고, 물체들 반환 시 NMS 수행 (3D IOU 계산 내장)
    # SceneMap은 동적 GSG 생성 연구에서 사용함. (2019.01.17 최초 개발)
    # DynamicGlobalSceneGraphManager는 동적 GSG 관리 매니저 (기존 run_thor의 기능을 옮김[for MVC]) (2019.01.24 최초 개발)
    # (190220) kips 논문 실험을 위해, 저장되는 물체 정보에 box2d 추가 (id 다음에 이어서 추가됨 obj[11:15])
'''
    ...
    def apply_action_model(self, inputs, merge_results=True):
        ...
        if not merge_results:
            ...
            if self.sceneMap.get_agent_object() is not None: # carry
                # 소유 물체 위치 변경 (sceneMap의 에이전트 정보는 인식 이후에 업데이트 되어서 지금 쓸 수 없음, 그래서 최신의 에이전트 정보를 따로 줘야함)
